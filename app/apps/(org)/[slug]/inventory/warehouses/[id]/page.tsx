...
import { deleteWarehouse } from "@/lib/services/inventory"; // You must have this API function, otherwise change accordingly
...
export default function WarehouseDetailPage() {
  ...
  const loadWarehouseDetails = async () => {
    try {
      ...
      const [warehouseData, inventoryData, statsData, movementsData] =
        await Promise.all([
    ...
    >
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Can permission={COMMON_PERMISSIONS.INVENTORY.UPDATE_WAREHOUSES}>
              <Button asChild>
                <Link
                  href={`/apps/${slug}/inventory/warehouses/${warehouseId}/edit`}
                >
            ...
            <Can permission={COMMON_PERMISSIONS.INVENTORY.DELETE_WAREHOUSES}>
              <Button
              disabled={(warehouse?.product_count ?? 0) > 0}
                type="button"
                variant="destructive"
                title="Supprimer l'entrepôt"
                onClick={() => setIsDeleteDialogOpen(true)}
              >
                Supprimer <Trash className="h-4 w-4" />
              </Button>
            </Can>
          </div>
        </div>
        ...
        <DeleteConfirmation
          ...
          title="Supprimer l'entrepôt"
          description={`Êtes-vous sûr de vouloir supprimer l'entrepôt "${warehouse.name}" ? Cette action est irréversible.`}
        ...
        <div className="grid gap-4 md:grid-cols-5">
          ...
          <Card className="p-4 text-center">
            ...
            <p className="text-2xl font-bold">
              {stats?.unique_products || inventory.length}
            </p>
          ...
          <Card className="p-4 text-center">
            ...
            <p className="text-2xl font-bold">
              {new Intl.NumberFormat("fr-FR").format(stats?.total_quantity || 0)}
            </p>
          ...
          <Card className="p-4 text-center">
            ...
            <p className="text-2xl font-bold">
              {formatCurrency(
                stats?.total_value || warehouse.total_stock_value || 0
              )}
            </p>
        ...
        {stats && stats.low_stock_products > 0 && (
          <Alert variant="warning" className="flex items-center gap-2 py-1 px-2 text-xs">
            <AlertTriangle className="h-3 w-3" />
            <span>
              <strong>{stats.low_stock_products}</strong> produit(s) en stock bas
            </span>
          </Alert>
        )}
                    ...
                    <div
                      ...
                      onClick={() =>
                        hasPermission(
                          ...
                          ? router.push(
                              `/apps/${slug}/inventory/products/${stock.product}`
                            )
                          : null
                      }
                    >
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <Can permission={COMMON_PERMISSIONS.INVENTORY.VIEW_PRODUCTS}>
                          ...
                          >
                            <Link
                              href={`/apps/${slug}/inventory/products/${stock.product}`}
                            >
            ...
            {movements.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground">
                  Pas encore de mouvement dans cet entrepôt
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                ...
                <div className="pt-4 text-center">
                  <Button variant="outline" size="sm" asChild>
                    <Link
                      href={`/apps/${slug}/inventory/movements?warehouse=${warehouseId}`}
                    >
